...
class RoomTypeDAL(BaseDal):
    ...
    def create_new_room_type(self, description: str, max_guests: int) -> model.RoomType:
        ...
        return model.RoomType(type_id=last_row_id, description=description, max_guests=max_guests)

    def read_room_type_by_id(self, room_type_id: int) -> model.RoomType | None:
        # Zimmertyp nach ID auslesen
        sql = "SELECT TypeId, Description, MaxGuests FROM RoomType WHERE TypeId = ?"
        result = self.fetchone(sql, (type_id,))
        if result:
            id_, desc, max_g = result
            return model.RoomType(type_id=id_, description=desc, max_guests=max_g)
        return None
    ...
    def read_all_room_types(self) -> list[model.RoomType]:
        ...
        return [model.RoomType(type_id=id_, description=desc, max_guests=max_g) for id_, desc, max_g in rows]

    def read_all_room_types_as_df(self) -> pd.DataFrame:
        # Alle Zimmertypen als DataFrame
        sql = "SELECT TypeId, Description, MaxGuests FROM RoomType"
        return pd.read_sql(sql, self.get_connection(), index_col="TypeId")

    def update_room_type(self, room_type: model.RoomType) -> None:
        # Zimmertyp aktualisieren
        sql = "UPDATE RoomType SET Description = ?, MaxGuests = ? WHERE TypeId = ?"
    ...
    def delete_room_type(self, room_type: model.RoomType) -> None:
        ...
        self.execute(sql, (room_type.type_id,))
